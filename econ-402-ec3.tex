...
\begin{equation}
\label{eq:13}
Y = C + I + G = \overline{C} + \text{MPC} Y + \overline{I} - \varepsilon r + G,
\end{equation}
where is explicitly in terms of $Y$ and $r$; recall that $G$ is exogenous.
...
\begin{align*}
Y &= \overline{C} + \text{MPC} Y + \overline{I} - \varepsilon r + G \\
...
\caption[]{\label{fig:is-lm} Allowing the LM curve to play a role.}
\end{figure}
